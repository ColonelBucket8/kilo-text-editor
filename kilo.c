...
int getWindowSize(int *rows, int *cols) {
  ...
  if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
    ...
    // Use large value like 999 to ensure the cursor reaches the right
...
void editorAppendRow(char *s, size_t len) {
  ...
  E.numrows++;
  // Could have used E.dirty = 1 instead of incrementing it ot show
  // how dirty it is
  E.dirty++;
}
...
void editorOpen(char *filename) {
  ...
  size_t linecap = 0;
  ssize_t linelen;
  // getline() is useful for reading lines from a file when we don’t know how
  // much memory to allocate for each line
  // getline returns -1 when it gets to then of the file
...
void editorSave() {
  ...
  // 0644 - gives the owner of the file permission to read
  // and write the file and everyone else only gets
...
void editorFindCallback(char *query, int key) {
  ...
  if (last_match == -1)
    direction = 1;
  // Current is the index of the current row we are searching
...
void editorScroll() {
  ...
  // if so, scrolls up to where the cursor is
  if (E.cy < E.rowoff) {
    E.rowoff = E.cy;
  }
...
void editorDrawRows(struct abuf *ab) {
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    // Only add new line if not last row
    abAppend(ab, "\r\n", 2);
  }
}
...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  while (len < E.screencols) {
    // Print the col and row line at the most right of status bar
  ...
  case CTRL_KEY('q'):
    if (E.dirty && quit_times > 0) {
      editorSetStatusMessage("WARNING!! File has unsaved changes. "
                             "Press Ctrl-! %d more times to quit.",
                             quit_times);
      quit_times--;
      return;
    }
